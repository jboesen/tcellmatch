...
class EstimatorBase:
    ...
    def _read_binarized_matrix_v3_0(
        ...
        #     covariates_table[x] = cell_table[x].values
        covariates_table = cell_table.set_index(id_col).copy()
        if fns_clonotype is not None:
    ...
    def _format_covariates_from_table(
        ...
        #         table_temp[new_id] = table_temp[x].values
            rename_dict = {x: strip_string(x) for x in table_temp.columns}
            table_temp.rename(columns=rename_dict, inplace=True)
